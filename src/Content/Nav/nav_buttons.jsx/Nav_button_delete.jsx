...
function Nav_button_delete({ firstUseRef, bool, articleID, setArticleData, setMenu }) {
  ...
  const deleteHandle = () => {
    firebase_delete(articleID.current);
    setModal(true);
    setMenu(false);
  };
  ...
  return (
    <>
      <button
        className={`nav-button4 nav-button_${firstUseRef.current}${bool} nav-button-animate-${hover}`}
        onMouseEnter={() => {
          setHover(true);
        }}
        onMouseLeave={() => {
...
export default Nav_button_delete;

Nav_button_delete.propTypes = {
  bool: PropTypes.bool,
